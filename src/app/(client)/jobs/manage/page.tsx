...
import { Eye, Users, MoreHorizontal } from "lucide-react";
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/client/jobs/new" className="bg-primary text-white px-5 py-2.5 rounded-xl text-[15px] font-medium hover:opacity-90 transition-opacity press-effect">
          + Post New Job
        </Link>
      </div>
      ...
      <div className="flex gap-2 mb-6">
        {["all", "open", "in_progress", "filled", "cancelled"].map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-4 py-2 rounded-full text-[13px] font-medium capitalize transition-colors press-effect ${
              filter === f ? "bg-primary text-white" : "bg-white text-foreground border border-border hover:bg-background"
            }`}
          >
            {f.replace("_", " ")}
          </button>
          ...
          {filtered.map((job) => (
            <div key={job.id} className="grid md:grid-cols-12 gap-4 px-6 py-5 items-center hover:bg-background/50 transition-colors">
              ...
              <div className="col-span-2">
                <span className={`inline-block px-3 py-1 rounded-full text-[12px] font-medium capitalize ${statusColors[job.status]}`}>
                  {job.status.replace("_", " ")}
                </span>
              </div>
              <div className="col-span-2">
                <p className="text-[15px] text-foreground">{job.applicants} applicants</p>
              </div>
              <div className="col-span-2">
                <p className="text-[15px] font-medium text-[#34C759]">{job.pay}</p>
              </div>
              <div className="col-span-2 flex items-center gap-2">
                <Link href={`/client/jobs/${job.id}/applicants`} className="p-2 text-muted-foreground hover:text-primary transition-colors" title="View Applicants">
                  <Users className="size-4" />
                </Link>
                {job.status === "open" && (
                  <button className="text-[13px] text-primary hover:underline">Close</button>
                )}
              </div>
            </div>
          ))}
