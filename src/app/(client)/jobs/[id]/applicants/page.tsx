...
import { ArrowLeft, Star, Phone, CreditCard, Shield, Check, X } from "lucide-react";
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto">
      <button onClick={() => router.back()} className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors mb-6 press-effect">
        ...
        <span className="text-[14px]">Back to jobs</span>
      </button>
      ...
      <div className="space-y-4">
        {mockApplicants.map((worker) => (
          <div key={worker.id} className={`bg-white rounded-2xl border p-6 transition-all ${
            selected === worker.id ? "border-primary ring-2 ring-primary/20" : "border-border"
          }`}>
            <div className="flex items-start justify-between mb-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="flex items-center gap-1 text-[13px]">
                      <Star className="size-3.5 text-[#FF9500]" fill="#FF9500" /> {worker.rating}
                    </span>
                    <span className="text-[13px] text-muted-foreground">· {worker.jobsCompleted} jobs</span>
                    {worker.cidVerified && (
                      <span className="apple-pill apple-pill-green text-[11px]">CID Verified</span>
                    )}
                  </div>
                </div>
              </div>
              <button
                ...
                className={`px-5 py-2.5 rounded-xl text-[14px] font-medium transition-all press-effect ${
                  selected === worker.id
            ...
            {contactUnlocked === worker.id && (
              <div className="bg-[#34C759]/5 border border-[#34C759]/20 rounded-2xl p-5 mt-4">
                <p className="text-[14px] font-semibold text-[#34C759] mb-3 flex items-center gap-2">
                  <Check className="size-4" /> Contact Unlocked
                </p>
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <Phone className="size-4 text-primary" />
                    <span className="text-[15px] font-medium text-foreground">{worker.phone}</span>
                  </div>
                  {worker.bankQR && (
                    <div className="flex items-center gap-3">
                      <CreditCard className="size-4 text-primary" />
                      <span className="text-[15px] text-foreground">{worker.bankQR}</span>
                    </div>
                  )}
                  <p className="text-[13px] text-muted-foreground mt-2">
                    Contact the worker directly for payment via mBoB or bank transfer.
                  </p>
                </div>
              </div>
            )}
